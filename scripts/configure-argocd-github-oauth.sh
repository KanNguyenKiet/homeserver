...
cleanup() {
  unset ROOT_TOKEN GITHUB_CLIENT_ID GITHUB_CLIENT_SECRET
}
...
[[ -n "$GITHUB_CLIENT_ID" ]] || fail "GitHub OAuth client ID cannot be empty"

read -r -s -p "GitHub OAuth client secret: " GITHUB_CLIENT_SECRET
printf '\n'
[[ -n "$GITHUB_CLIENT_SECRET" ]] || fail "GitHub OAuth client secret cannot be empty"

log "Configuring the Vault policy, Kubernetes auth role, and OAuth credentials"
{
  printf '%s\n' "$ROOT_TOKEN"
  printf '%s\n' "$GITHUB_CLIENT_ID"
  printf '%s\n' "$GITHUB_CLIENT_SECRET"
  ...
  IFS= read -r GITHUB_CLIENT_SECRET
  ...
  vault policy write argocd-github-oauth-read - >/dev/null <<POLICY
path "kv/data/homeserver/argocd" {
  capabilities = ["read"]
}

path "kv/metadata/homeserver/argocd" {
  capabilities = ["read"]
}
POLICY

  vault write auth/kubernetes/role/argocd \
    bound_service_account_names=argocd-vault-auth \
    bound_service_account_namespaces=argocd \
    audience=vault \
    policies=argocd-github-oauth-read \
    ttl=1h >/dev/null

  printf "%s" "$GITHUB_CLIENT_ID" | \
    vault kv put -mount=kv homeserver/argocd githubClientID=- >/dev/null
  printf "%s" "$GITHUB_CLIENT_SECRET" | \
    vault kv patch -mount=kv homeserver/argocd githubClientSecret=- >/dev/null

  unset VAULT_TOKEN GITHUB_CLIENT_ID GITHUB_CLIENT_SECRET
'

unset ROOT_TOKEN GITHUB_CLIENT_ID GITHUB_CLIENT_SECRET

log "Waiting for External Secrets to create the Argo CD OAuth Secret"
kubectl wait --for=create secretstore/vault-backend -n argocd \
  --timeout="$WAIT_TIMEOUT"
kubectl wait --for=create externalsecret/argocd-github-oauth -n argocd \
  --timeout="$WAIT_TIMEOUT"
kubectl annotate externalsecret argocd-github-oauth -n argocd \
  external-secrets.io/force-sync="$(date +%s)" --overwrite
kubectl wait secretstore/vault-backend -n argocd \
  --for=condition=Ready --timeout="$WAIT_TIMEOUT"
kubectl wait externalsecret/argocd-github-oauth -n argocd \
  --for=condition=Ready --timeout="$WAIT_TIMEOUT"

log "Restarting Argo CD authentication components to load the OAuth credentials"
...
log "Argo CD GitHub OAuth is ready"
